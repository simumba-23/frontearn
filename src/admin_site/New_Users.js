...
const NewUsers = () => {
  ...
  const { fetch_admin_reports, bulkActions } = useApi();
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        if (response.data && response.data.display_new_users) {
          setData(response.data.display_new_users);
          console.log(response.data.display_new_users);
        } else {
          setData([]);
        }
      } catch (error) {
              ...
              <tbody>
                {currentPageData.map((user) => (
                  <tr key={user.id}>
                    ...
                    <td>{user.id}</td>
                  <td as ={Link} to={ `/user/${id}/detail`} style={{ textDecoration:'none'}}>{user.username}</td>
